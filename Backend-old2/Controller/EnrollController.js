  ...
  async getEnrolls(req, res) {
    try {
      ...
      let allEnrolls = await Enrolled.find({ userId: uid })
        .populate({
          path: "userId",
          select: "name email", // Only include specific fields from Users model
        })
        .populate({
          path: "courseId",
          select: "name description poster price duration", // Only include specific fields from Courses model
        });
