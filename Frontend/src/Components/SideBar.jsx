...
import { Home, BookOpen, Coffee, Mail, Info ,HelpCircle  } from 'lucide-react';
    ...
    `}>
      ...
      <nav className="flex-1 overflow-y-auto py-4">
        <ul className="space-y-1 px-3">
          ...
          <li>
            <Link to="/doubt" className={`
                flex items-center py-2 px-3 rounded-lg transition-colors
                ${isActive("/donuts") ? 'bg-indigo-50 text-indigo-700 font-medium' : 'text-gray-700 hover:bg-gray-100'}
                ${isCollapsed ? 'justify-center' : ''}
              `} title={isCollapsed ? "Donuts" : ""}>
              <HelpCircle  size={20} className={isActive("/doubt") ? 'text-indigo-600' : 'text-gray-500'} />
              {!isCollapsed && <span className="ml-3">doubt</span>}
            </Link>
          </li>
          <li>
